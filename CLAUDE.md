...
**阶段**:W5–6 CLI A 层（进行中,冒烟测试受 API 403 阻塞）
**目标**:A 层实现、353 词术语表和 400 条 CLI SAFE 审阅已完成;S1 通过,S2–S3 待账号请求恢复后复测
**本阶段禁止**:在 S1–S3 全绿前发布、合并或进入 W7–8;禁止修改 Claude Code 程序、登录或凭证配置
**下一阶段**:解除 API 403 后重跑 S2–S3,全绿后进入 W7–8 版本监控与流水线
